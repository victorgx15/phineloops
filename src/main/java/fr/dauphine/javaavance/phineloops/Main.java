...
public class Main {
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        options.addOption("i", "inverse ", true, "Generate an inverse problem grid of size height x width.");
        ...
        //Option pour générer l'interface graphique
        
        options.addOption("m","menu",true,"Display application menu");
    ...
    try{    
        if( cmd.hasOption( "g" ) ) {
            ...
            Fichier.genererFichierSortie(g,outputFile);
        }
        else  if( cmd.hasOption( "i" ) ) {
            System.out.println("Running phineloops generator of inverse problem.");
            String[] gridformat = cmd.getOptionValue( "i" ).split("x");
            width = Integer.parseInt(gridformat[0]);
            height = Integer.parseInt(gridformat[1]); 
            if(! cmd.hasOption("o")) throw new ParseException("Missing mandatory --output argument.");
            outputFile = cmd.getOptionValue( "o" );
            ...
            Grille g=Fichier.lireFichierEntree(inputFile);
            solved=g.resolution();
            Fichier.genererFichierSortie(g, outputFile);
        ...
        {
            System.out.println("Running application menu");
            String[] gridformat = cmd.getOptionValue( "m" ).split("x");
	    try{
		width = Integer.parseInt(gridformat[0]);
            height = Integer.parseInt(gridformat[1]);
		if((width<=0) || (height<=0) || (width>8) || (height>8))
		{
			System.out.println("Dimension inadaptée !");
		}	
		else	
		{
            Menu m=new Menu();
		...
		{
			System.out.println("Dimension incorrecte !");
		}
        }
        else {
